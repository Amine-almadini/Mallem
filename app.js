...
document.addEventListener("click", (event) => {
  ...
  const actions = {
    ...
    showPro: () => { const pro = state.users.find(u => u.id === id); if (pro) renderProModal(pro); },
    closeProModal: () => { const el = document.getElementById("pro-modal"); if (el) el.remove(); },
    openGallery: () => { const pro = state.users.find(u => u.id === id); if (pro) renderGallery(pro, parseInt(value, 10)); },
    closeGallery: () => { const el = document.getElementById("gallery-overlay"); if (el) el.remove(); },
    galleryPrev: () => { const el = document.getElementById("gallery-overlay"); if (el) el.remove(); const pro = state.users.find(u => u.id === id); if (pro) renderGallery(pro, Math.max(0, parseInt(value, 10) - 1)); },
    galleryNext: () => { const el = document.getElementById("gallery-overlay"); if (el) el.remove(); const pro = state.users.find(u => u.id === id); if (pro) renderGallery(pro, Math.min((pro.workPhotos || []).length - 1, parseInt(value, 10) + 1)); }
  };
...
function proCard(pro) {
  ...
  return `
    <article class="card pro-card lift">
      <div class="pro-head">
        <div class="avatar">${pro.photoURL ? `<img src="${esc(pro.photoURL)}" alt="">` : esc(name[0] || "م")}</div>
        <div>
      ...
      <div style="display:flex;gap:8px">${state.user ? `<button class="btn ghost" data-action="reviewPro" data-id="${esc(pro.id)}" data-value="${esc(pro.whatsapp || pro.phone)}" data-name="${esc(name)}" style="flex:1">تقييم و إرسال ملاحظات</button>` : ""}<button class="btn turquoise" data-action="showPro" data-id="${esc(pro.id)}" style="${state.user ? 'flex:0 0 auto' : 'width:100%'}">عرض</button></div>
    </article>`;
}

function renderProModal(pro) {
  const existing = document.getElementById("pro-modal");
  if (existing) existing.remove();
  const name = displayName(pro);
  const badge = pro.verified ? "معتمد" : pro.verificationStatus === "pending" ? "قيد المراجعة" : "تقليدي";
  const rating = pro.rating ? `${"★".repeat(Number(pro.rating))}${"☆".repeat(5 - Number(pro.rating))}` : "لا يوجد";
    ...
    <div class="modal" role="dialog">
      <div class="modal-head">
        <div style="display:flex;align-items:center;gap:14px">
          <div class="avatar" style="width:64px;height:64px;font-size:28px">${pro.photoURL ? `<img src="${esc(pro.photoURL)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` : esc(name[0] || "م")}</div>
          <div>
            <h2 style="margin:0">${esc(name)}</h2>
            <div style="margin-top:4px">${pro.verified ? '<span class="verified-badge">معتمد</span>' : pro.verificationStatus === "pending" ? '<span class="badge-warn" style="font-size:11px;padding:2px 8px;border-radius:999px">قيد المراجعة</span>' : '<span class="trad-badge">تقليدي</span>'}</div>
          </div>
        </div>
        <button class="btn ghost" data-action="closeProModal">✕</button>
      </div>
      <div class="modal-body">
        <div class="field-group">
          <div class="field"><label>الهاتف</label><span>${esc(pro.phone || "")}</span></div>
          <div class="field"><label>واتساب</label><span>${esc(pro.whatsapp || pro.phone || "")}</span></div>
          <div class="field"><label>العنوان</label><span>${esc(pro.address || "")}</span></div>
          <div class="field"><label>المدينة</label><span>${esc(pro.city || "")}</span></div>
          <div class="field"><label>الحرفة</label><span>${esc(pro.job || "")}</span></div>
          <div class="field"><label>التقييم</label><span style="color:#f59e0b">${rating}</span></div>
          <div class="field"><label>الحالة</label><span>${badge}</span></div>
        </div>
        ${(pro.workPhotos || []).length ? `
        <div class="gallery-section">
          <label class="gallery-label">صور الأعمال</label>
          <div class="gallery-thumbs">${pro.workPhotos.map((url, i) => `<img src="${esc(url)}" alt="" class="gallery-thumb" data-action="openGallery" data-id="${esc(pro.id)}" data-value="${i}">`).join("")}</div>
        </div>` : ""}
      </div>
    </div>`;
...
function renderGallery(pro, index) {
  const existing = document.getElementById("gallery-overlay");
  if (existing) existing.remove();
  const photos = pro.workPhotos || [];
  if (!photos.length) return;
  const div = document.createElement("div");
  div.className = "gallery-overlay";
  div.id = "gallery-overlay";
  div.innerHTML = `
    <button class="gallery-close" data-action="closeGallery">✕</button>
    <button class="gallery-nav gallery-prev" data-action="galleryPrev" data-id="${esc(pro.id)}" data-value="${index}">‹</button>
    <div class="gallery-image-wrap"><img src="${esc(photos[index])}" alt="" class="gallery-image" id="gallery-img"></div>
    <button class="gallery-nav gallery-next" data-action="galleryNext" data-id="${esc(pro.id)}" data-value="${index}">›</button>
  ...
  const keyHandler = (e) => {
    if (e.key === "Escape") { div.remove(); document.removeEventListener("keydown", keyHandler); }
    if (e.key === "ArrowLeft") { const next = Math.max(0, index - 1); if (next !== index) { div.remove(); document.removeEventListener("keydown", keyHandler); renderGallery(pro, next); } }
    if (e.key === "ArrowRight") { const next = Math.min(photos.length - 1, index + 1); if (next !== index) { div.remove(); document.removeEventListener("keydown", keyHandler); renderGallery(pro, next); } }
  };
...
function accountInfo(u) {
  return `
    <div class="profile-head">
      <div class="avatar big">${u.photoURL ? `<img src="${esc(u.photoURL)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` : esc(displayName(u)[0] || "م")}</div>
      <div>
